...
public class ReservationManager {
	...
	public ReservationManager(ReservationDao reservationDao, RoomDao roomDao, ArrayList<Reservation> reservationList, ArrayList<Room> roomList)
	{ 
	...
	public void updateReservation()
	{
		...
		switch(choice)
		{
			...
			case 2: 
					...
					{
						if(reservationList.get(i).getRoom().getRoomNumber()==reservation.getRoom().getRoomNumber())
						{
							room = this.getRoomByRoomNum(reservationList.get(i).getRoom().getRoomNumber());
	...
	public void displayAllReservation()
	{
		for(int i=0;i<reservationList.size();i++)
		{	
			String reservationCode = reservationList.get(i).getReservationCode();
			String identity = reservationList.get(i).getIdentity();
			Room room = reservationList.get(i).getRoom();
			double price = reservationList.get(i).getPrice();
			Date reservationDatetime = reservationList.get(i).getReservationDatetime();
			Date checkInDatetime = reservationList.get(i).getCheckInDatetime();
			Date checkOutDatetime = reservationList.get(i).getCheckOutDatetime();
			int numOfAdult = reservationList.get(i).getNumOfAdult();
			int numOfChildren = reservationList.get(i).getNumOfChildren();
			ReservationStatus reservationStatus = reservationList.get(i).getReservationStatus();
	...
	private String getCurrentDateTime()
	{
		...
		String currentDateTime = new StringBuilder().append(year).append(twoint.format(month)).append(twoint.format(day)).append(twoint.format(hour)).append(twoint.format(min)).append(twoint.format(sec)).toString();
	...
	{	
		...
		for(int i=0;i<reservationList.size();i++)
		{
			if(reservationList.get(i).getCheckInDatetime().compareTo(current)<0 && !reservationList.get(i).getReservationStatus().equals(ReservationStatus.returnStatus(4)))
			{
				reservationList.get(i).setReservationStatus(ReservationStatus.returnStatus(4));
				Room room = this.getRoomByRoomNum(reservationList.get(i).getRoom().getRoomNumber());
				this.updateStatusBySystem(room, RoomStatus.returnStatus(1));
			}
		}
	}
