...
public class BillManager {
	 ...
	 * 
	 * @param roomNum
	 *            The room number of the new guest.
	 */
	public void createBill(int roomNum) {
		Bill newBill = new Bill(billDao.getAllBill().size() + 1, roomNum, 0, 0,
				0);
		billDao.addBill(newBill);
	}

	/**
	 * Allows the order room service manager to update the total room service
	 * charge when the guest adds a new order.
	 * 
	 * @param roomNum
	 *            The room number of guest that ordered a room service.
	 * @param newPrice
	 *            The price of the room service ordered.
	 */
	 ...
	 * 
	 * @param roomNum
	 *            The room number of the guest that checks out.
	 */
	 ...
	 * 
	 * @param roomNum
	 *            The room number of the bill to be displayed.
	 */
	public void displayBillbyRoomNum(int roomNum) {
		for (int i = 0; i < billDao.getAllBill().size(); i++) {
			if (billDao.getAllBill().get(i).getRoomNum() == roomNum) {
				System.out.println("Bill Number : "
						+ billDao.getAllBill().get(i).getBillNum() + "\t\t"
						+ "Room Number : "
						+ billDao.getAllBill().get(i).getRoomNum() + "\t\t"
						+ "Room Charge : "
						+ billDao.getAllBill().get(i).getRoomCharge() + "\t\t"
						+ "Room Service Charge : "
						+ billDao.getAllBill().get(i).getRoomServiceCharge()
						+ "\t\t" + "Discount : "
						+ billDao.getAllBill().get(i).getDiscount());
	...
	/**
	 * Returns the room service data access object so that other manager class
	 * can access it.
	 * 
	 * @return the bill data access object.
	 */
