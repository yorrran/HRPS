...
public class PaymentManager {
	 ...
	 * 
	 * @param billManager
	 *            The bill manager to get the amount of charges.
	 * @param orderManager
	 *            The list of room service ordered.
	 */
	 ...
	 * @param days
	 *            The number of days the guest stayed in the room.
	 */
	...
	/**
	 * The total amount that the guest should pay is calculated. If the guest
	 * chooses to pay by credit card, the process will be automated.
	 * 
	 * @param roomNum
	 *            The room number of the guest.
	 * @param days
	...
	public void byCreditCard(int roomNum, int days) {
		for (int i = 0; i < billManager.getBillDao().getAllBill().size(); i++) {
			if (billManager.getBillDao().getAllBill().get(i).getRoomNum() == roomNum) {
				double total = billManager.getBillDao().getAllBill().get(i)
						.getRoomCharge()
						+ billManager.getBillDao().getAllBill().get(i)
								.getRoomServiceCharge();
				double discount = total
						* billManager.getBillDao().getAllBill().get(i)
								.getDiscount();
				double taxable = billManager.getBillDao().getAllBill().get(i)
						.getTax()
						* total;
				...
				for (int j = 0; j < orderManager.getOrderDao()
						.getOrderByRoomNum(roomNum).getRoomServiceList().size(); j++) {
					orderItems += "\t"
							+ (i + 1)
							+ ". "
							+ orderManager.getOrderDao()
									.getOrderByRoomNum(roomNum)
	...
	/**
	 * The total amount that the guest should pay is calculated. If the guest
	 * chooses to pay by cash, enter the amount of cash paid by the guest.
	 * 
	 * @param roomNum
	 *            The room number of the guest.
	 * @param days
