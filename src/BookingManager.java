...
public class BookingManager {
	 ...
	 *            The payment manager.
	 */
	public BookingManager(GuestManager guestManager,
			ReservationManager reservationManager, RoomManager roomManager,
			BillManager billManager, OrderManager orderManager,
	...
	public void addBooking() {
		...
		if (cus_type == 1) {
			...
			for (i = 0; i < guestManager.getGuestDao().getAllGuest().size(); i++) {
				if (guestManager.getGuestDao().getAllGuest().get(i)
						.getIdentity().equals(cus_id)) {
	...
	private void walkinCustomer(String cus_id) {
		...
		do {
			...
			if (checkInDate.compareTo(checkOutDate) >= 0)
				System.out
						.println("Check in date must be earlier than check out date");
		} while (checkInDate.compareTo(checkOutDate) >= 0);

		int numOfDays = checkOutDate.getDate() - checkInDate.getDate();
		availableRoomList = new ArrayList<Room>(roomManager.getAllRoom());// make
																			// a
																			// copy
																			// of
																			// room
		...
		reservationManager.updateAvailableRoomList(availableRoomList, tempDate,
				checkOutDate);
		for (int i = 0; i < roomManager.getAllRoom().size(); i++) {
			if (roomManager.getAllRoom().get(i).getRoomStatus()
					.equals(RoomStatus.returnStatus(2))) {
				for (int j = 0; j < availableRoomList.size(); j++) {
					if (availableRoomList.get(j).getRoomNumber() == roomManager
							.getAllRoom().get(i).getRoomNumber())
		...
		case 1:
			do {
				...
				for (int i = 0; i < availableRoomList.size(); i++) {
					if (room.getRoomNumber() == availableRoomList.get(i)
							.getRoomNumber()) {
				...
				if (foundAvailableRoom) {
					if (room.getRoomStatus().equals(RoomStatus.returnStatus(4)))// not
																				// allow
																				// to
																				// book
																				// if
					...
					{
						System.out
								.println("This room is unavailable now, please select another room.");
						room = null;
					}
				} else {
					System.out
							.println("The room is unavailable in that period, please select another room.");
					room = null;
				}
			} while (room == null);// select the vacant room

			double price = (room.getRoomType().getPrice() + room.getFacing()
					.getPrice());// get price for room type and facing type
			Booking booking = new Booking(cus_id, room, price, checkInDate,
			...
			roomManager.getRoomByRoomNum(room.getRoomNumber()).setRoomStatus(
					RoomStatus.returnStatus(2));

			String reservationCode = new StringBuilder()
					.append(getCurrentDateTime()).append(cus_id).toString();
			Reservation reservation = new Reservation(reservationCode, cus_id,
					room, price, checkInDate, checkInDate, checkOutDate,
					numberOfAdults, numberOfChildren,
					ReservationStatus.returnStatus(3));
	...
	private void reservedCustomer(String reservation_code) {
		Reservation reservation = reservationManager
				.searchReservationbyReservationCode(reservation_code);
		...
		if (reservation != null) {
			...
			} else if (reservation.getReservationStatus().equals(
					ReservationStatus.returnStatus(4))) {
				System.out.println("The reservation is expired");
			} else {
				if (reservation.getReservationStatus().equals(
						ReservationStatus.returnStatus(3))) {
					System.out.println("Customer has already checked in");
				} else {
					Booking booking = new Booking(reservation.getIdentity(),
							roomManager.getRoomByRoomNum(reservation.getRoom()
									.getRoomNumber()), reservation.getPrice(),
							reservation.getCheckInDatetime(),
							reservation.getCheckOutDatetime(),
							reservation.getNumOfAdult(),
							reservation.getNumOfAdult());
					roomManager.getRoomByRoomNum(
							reservation.getRoom().getRoomNumber())
							.setRoomStatus(RoomStatus.returnStatus(2));
					reservation.setReservationStatus(ReservationStatus
							.returnStatus(3));
					billManager.createBill(reservation.getRoom()
							.getRoomNumber());
					orderManager.createOrder(reservation.getRoom()
							.getRoomNumber());
					bookingDao.addBooking(booking);
				}
			}
	 ...
	 * 
	 * @param roomNum
	 *            The room number that need to be updated
	 * @param cus_id
	 *            The customer identity that makes booking
	 */
		...
		if (current.after(booking.getCheckOutDatetime())) {
			...
			case 1:
				for (int i = 0; i < bookingDao.getAllBooking().size(); i++) {
					if (bookingDao.getAllBooking().get(i).getRoom()
							.getRoomNumber() == roomNum) {
						if (bookingDao.getAllBooking().get(i).getIdentity()
								.equals(cus_id)) {
							System.out
									.println("How many days that need to be extended");
							int extendDays = Input.GetInt();
							Date checkInDate = reservationManager
									.getReservationDao().getAllReservation()
									.get(i).getCheckInDatetime();
							Date checkOutDate = reservationManager
									.getReservationDao().getAllReservation()
									.get(i).getCheckOutDatetime();
							int diff = (int) (checkOutDate.getDate() - checkInDate
									.getDate());
							...
							for (int j = 0; j < reservationManager
									.getReservationDao().getAllReservation()
									.size(); j++) {
								if (reservationManager.getReservationDao()
										.getAllReservation().get(j).getRoom()
										.getRoomNumber() == roomNum
										&& !reservationManager
												.getReservationDao()
												.getAllReservation().get(j)
												.getIdentity().equals(cus_id)) {
									if (newCheckOutDays
											.after(reservationManager
													.getReservationDao()
													.getAllReservation().get(j)
													.getCheckInDatetime())
											&& newCheckOutDays
								...
								if (!status) {
									System.out
											.println("The room is reserved by other customers. Would you like to book another room?");
									...
									if (option == 1) {
										...
										System.out
												.println("The updation is been cancelled");
	...
	/**
	 * Print out specific booking details by using room number
	 * 
	 * @param roomNum
	 *            The room number
	 */
	...
	public void checkOut() {
		...
		case 1:
			...
			roomManager.getRoomByRoomNum(roomNum).setRoomStatus(
					RoomStatus.returnStatus(1));

			if (booking != null) {
				numOfDays = booking.getCheckOutDatetime().getDate()
						- booking.getCheckInDatetime().getDate();
				price = booking.getPrice() * numOfDays;

				for (int i = 0; i < reservationManager.getReservationDao()
						.getAllReservation().size(); i++) {
					if (reservationManager.getReservationDao()
							.getAllReservation().get(i).getRoom()
							.getRoomNumber() == roomNum
							&& reservationManager.getReservationDao()
									.getAllReservation().get(i)
									.getReservationStatus()
									.equals(ReservationStatus.returnStatus(3)))
						reservationManager
								.getReservationDao()
								.getAllReservation()
								.get(i)
		...
		case 2:
			...
			for (int i = 0; i < reservationManager.getReservationDao()
					.getAllReservation().size(); i++) {
				if (reservationManager.getReservationDao().getAllReservation()
						.get(i).getRoom().getRoomNumber() == roomNum
						&& reservationManager.getReservationDao()
								.getAllReservation().get(i)
								.getReservationStatus()
								.equals(ReservationStatus.returnStatus(3))) {
	...
	public void autoCheckOut() {
		...
		for (int i = 0; i < bookingDao.getAllBooking().size(); i++) {
			...
			if (bookingDao.getAllBooking().get(i).getCheckOutDatetime()
					.after(current)) {
				System.out.print("The bookig is expired");
				roomManager.getRoomByRoomNum(
						bookingDao.getAllBooking().get(i).getRoom()
								.getRoomNumber()).setRoomStatus(
						RoomStatus.returnStatus(1));
				int numOfDays = bookingDao.getAllBooking().get(i)
						.getCheckOutDatetime().getDate()
						- bookingDao.getAllBooking().get(i)
								.getCheckInDatetime().getDate();
				double price = bookingDao.getAllBooking().get(i).getPrice()
						* numOfDays;
				bookingDao.removeBooking(bookingDao.getAllBooking().get(i));// only
																			// allow
																			// to
																			// remove
																			// when
				...
				System.out
						.println("This booking cannot be removed in this moment");
			}
		}
	}
	...
	private String getCurrentDateTime() {
		...
		String currentDateTime = new StringBuilder().append(year)
				.append(twoint.format(month)).append(twoint.format(day))
				.append(twoint.format(hour)).append(twoint.format(min))
				.append(twoint.format(sec)).toString();
